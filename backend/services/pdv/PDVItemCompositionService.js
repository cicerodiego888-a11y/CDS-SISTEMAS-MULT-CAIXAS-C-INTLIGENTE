/**
 * Motor de composição de linhas do carrinho do PDV.
 * Decide UNIFICAR / SEPARAR / AUTOMATICO sem alterar fiscal, estoque ou pagamento.
 */
'use strict';
...
/**
 * UNIFICAR — comportamento legado: mesmo produto_id + tipo_venda.
 */
function linhasCompativeisUnificar(linha, candidato) {
  if (!linha || !candidato) return false;
  return mesmoProduto(linha.produto_id ?? linha.id, candidato.produto_id ?? candidato.id)
...
function chaveComercialLinha(linha) {
  const etiqueta = linha && linha.etiqueta_balanca && typeof linha.etiqueta_balanca === 'object'
    ...
    : null;
  return {
    produto_id: Number(linha.produto_id ?? linha.id),
...
/**
 * AUTOMATICO — unifica só quando condições comerciais relevantes são compatíveis.
 */
...
/**
 * Decide se adiciona nova linha ou unifica em linha existente.
 * Não altera fiscal, estoque nem pagamento — só composição comercial do carrinho.
 */
...
/**
 * Soma quantidades do mesmo produto/tipo no carrinho (estoque visual vs comercial).
 */
function somarQuantidadeProduto(carrinho, produtoId, tipoVenda) {
  ...
  for (let i = 0; i < lista.length; i += 1) {
    const item = lista[i];
    if (!mesmoProduto(item.produto_id ?? item.id, produtoId)) continue;
